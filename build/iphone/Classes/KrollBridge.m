...
NSString * pushnotificationtest$ModuleRequireFormat = @"(function(exports){"
...
@implementation pushnotificationtestObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_pushnotificationtest);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create pushnotificationtest global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "pushnotificationtest" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_pushnotificationtest = [[pushnotificationtestObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_pushnotificationtest];
	
	NSString *_pushnotificationtestNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _pushnotificationtestNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_pushnotificationtest valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:pushnotificationtest$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.pushnotificationtest.kroll" 
