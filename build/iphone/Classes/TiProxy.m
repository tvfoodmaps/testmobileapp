...
//Rare exceptions to indicate a bug in the _pushnotificationtest code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_pushnotificationtest = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_pushnotificationtest]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.pushnotificationtest.module"
